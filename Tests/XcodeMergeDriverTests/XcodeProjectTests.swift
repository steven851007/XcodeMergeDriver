...
final class XcodeProjectTests: XCTestCase {
    ...
    func test_hasConflict_returnsTrueWhenConflictExist() throws {
        let conflictProject = try XcodeProject(content: conflict)
        
        XCTAssert(conflictProject.hasConflict)
        XCTAssert(conflictProject.pbxBuildFile.hasConflict)
    }
    
    func test_hasUnsupportedConflict() throws {
        XCTAssertThrowsError(try XcodeProject(content: unsupportedConflict))
    }
}

private let unsupportedConflict = """
// !$*UTF8*$!
{
